...
public class MongoURIHandlerImpl extends URIHandlerImpl
{
	...
	public MongoURIHandlerImpl(IMongoLocator mongoLocator, IQueryEngine queryEngine)
	{
		...
		dbObjectBuilderFactory = builderFactory;
		eObjectBuilderFactory = builderFactory;
	...
	public OutputStream createOutputStream(final URI uri, final Map<?, ?> options) throws IOException
	{
		...
		// TODO why are we calling getConverterService() here - seems redundant with the call below
		getConverterService();

		return outputStreamFactory.createOutputStream(getConverterService(), dbObjectBuilderFactory, uri, options, getCollection(uri, options), getResponse(options));
	}

	@Override
	public InputStream createInputStream(final URI uri, final Map<?, ?> options) throws IOException
	{
		return inputStreamFactory.createInputStream(getConverterService(), eObjectBuilderFactory, queryEngine, uri, options, getCollection(uri, options), getResponse(options));
	}
...
//			dbCollection.setReadPreference(new ReadPreference.TaggedReadPreference(tags));
	...
	private IConverterService getConverterService()
	{
		if (converterService == null)
			converterService = converterServiceFactory.createConverterService();
